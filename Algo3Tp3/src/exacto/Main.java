...
public class Main {
	...
	/**
	 Imagen de Ejemplo:
	<a href="http://www.york.cuny.edu/~malk/geometricstructures/2011-set%20E1.jpg" target="_blank">
   			<img src=http://www.york.cuny.edu/~malk/geometricstructures/2011-set%20E1.jpg alt="GrafoEjemplo1" width="270" height="168">
    <//a>
	 */
	public static void runExample1(){
		...
		int peso = main.carteroChino(grafo);
	...
	public Integer carteroChino(GrafoNodos grafo){
		...
		//Si entra acá es que tiene dos nodos de grado impar ACA Hace falta adaptarlo.
...
//		System.out.println(CarteroChinoTest.printList(recorrido));
	 ...
	 * @complejidadTemporal 
	 * @precondicion: Todos los nodos tienen que tener grado par.
	 *  
	 */
    ...
    public static class GrafoEjes {
    	...
    	public Integer complejidadTemporal = 0; //Cantidad de operaciones
		public void sumarComplejidad(){
			complejidadTemporal++;
		}
    	
        protected Map<Integer, List<Eje>> mapaAdyacencia = new HashMap();
        protected Map<Integer, Boolean> nodosVisibles = new HashMap();
        ...
        public void mostrarTodosLosEjes(){
        	for (Integer nodo : getNodos()) {
				for (Eje eje : ejes(nodo)) {
					mostrarEje(eje);
				}
			}
        }
        ...
        public List<Integer> getNodosGradoImpar(){
        	...
        	for (Integer nodo : mapaAdyacencia.keySet()) {
        		...
        		if(mapaAdyacencia.get(nodo) != null){
        			grado = mapaAdyacencia.get(nodo).size();
        		}
        ...
        public Eje getUnEje(){
        	...
        	for (Integer nodo : mapaAdyacencia.keySet()) {
        		List<Eje> unEje = mapaAdyacencia.get(nodo);
        		for (Eje eje : unEje) {
					if(eje.visible){
						return eje;
					}
        ...
        public boolean esConexoConBFS(){
        	
        	HashMap<Integer,Boolean> nodosMarcados = new HashMap<Integer, Boolean>();  
        	Eje unEje = getUnEje(); //O(n)
        	if(unEje != null){
        		int cantMarcados = marcarNodoEHijos(unEje.nodo1, nodosMarcados);
        		return (cantMarcados == cantNodos);
        	}
        ...
        private int marcarNodoEHijos(int nodo, HashMap nodosMarcados){
        	
        	int cantMarcados = 0;
        	nodosMarcados.put(nodo,true);
        	cantMarcados++;
        	for (Eje eje: ejes(nodo)) {
        		
        		boolean estaMarcado = false; 
        		if(nodosMarcados.get(eje.nodo2) != null){
        			estaMarcado = (Boolean)nodosMarcados.get(eje.nodo2);	
        		}
        		
        		if(!estaMarcado){
        			cantMarcados+= marcarNodoEHijos(eje.nodo2, nodosMarcados);
        		}
			}
        	return cantMarcados;
        }
        ...
        public boolean esConexo(){
        	...
        	Integer nodo;
        	Eje unEje = getUnEje(); //O(n)
        	Set<Integer> nodosEncontrados = new HashSet<Integer>();
        	List<Eje> aExplorar = new ArrayList<Eje>();
        	List<Eje> ejesAdy;
        	
        	if(unEje != null){
	        	nodosEncontrados.add(unEje.nodo1);
	        	aExplorar.add(unEje);
	        	nodo = unEje.nodo1;
	        	
	        	while(!aExplorar.isEmpty()){ //O(n-1)
	        		
	        		unEje = aExplorar.get(0);
	        		nodo = unEje.nodo2;
	        	...
	        	mostrarTodosLosEjes();
	        	return nodosEncontrados.size() == cantNodos;
        	}
        	mostrarTodosLosEjes();
	        return cantNodos==1;
        }
    ...
    public static class Eje{
    	...
    	boolean visible;
    	Integer peso;
    	int orientado; //0 - No orientado, 1 - Hacia adelante nodo1, nodo2, 2 - Hacia atras nodo2, nodo1
    	...
    	public Eje(Integer nodo1, Integer nodo2, Integer peso, boolean orientada){
    		this(nodo1,nodo2);
    		setPeso(peso);
    	}
	...
	public static class GrafoNodos extends GrafoEjes{
		...
		public void eliminarNodosGradoCero(){
			for (Integer nodo: getNodos()) {
				if(gradoNodo(nodo) %2==0){
					mapaAdyacencia.put(nodo, null);
					nodosVisibles.put(nodo, false);
				}
			} 
		}
