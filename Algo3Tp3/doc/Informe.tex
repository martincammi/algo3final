...
Este fue el puntapie inicial para un tipo de problema que m\'as adelante se modelar\'ia usando la teor\'ia de grafos. Este problema fue caracterizado por Euler y consist\'ia en encontrar un circuito que pasara por todas las aristas de un grafo una sola vez volviendo al punto de partida.\\
...
El segundo m\'etodo se basaba en el parametro para darle la longitud a la lista y no armaba la lista en base al cumplimiento de una condici\'on (ej porcentaje de ALFA) decidimos descartarlo tambi\'en.\\
...
Un Matching est\'a compuestos por pares de la forma (nodoOrigen, nodoDestino).
...
Esta definici\'on de vecindad tiene un tama\~{n}o de $\frac{k \cdot (k-1)}{2}$ donde $k$ es la cantidad de pares en el matching. Esto puede verse ya que para el intercambio de un par con otro existen $k-1$ posibilidades de otros pares. A su vez como la cantidad de pares en el matching es est\'a determinada por la diferencia de grados de entrada y salida de los nodos \'esta equivale a\\
...
\begin{algorithm}
	...
	\IF{ es fuertemente Conexo $GrafoOriginal$}
		...
		\WHILE{ no se alcanzan $MaximoIteracionesGrasp$ $\land$ $MaximoIteracionesSinMejoraGrasp$}
			...
			\WHILE{ no se alcanza $MaximoIteracionesBusquedaLocal$ $\land$ no se encontro un matching �ptimo}
