...
\end{verbatim}	
	...
	El costo del algoritmo es de O(n/2) * O(verificarPrimos(n)) donde
	
	Sea ci = el número impar i;
	
	Sea \verb+c_i+ = el candidato i-ésimo.
	...
	Si n/2 es par: \\
	verificarPrimos(n) = $\sum_{i = 1}^{n+4/4}{esPrimo(2*i+1)+esPrimo(n-2*i+1)}$
	
	Como esPrimo(n) = O($\sqrt(n)$)
	
	verificarPrimos(n) = $\sum_{i = 1}^{n+4/4}{\sqrt{(2*i+1)} + \sqrt{(n-(2*i+1))}}$
	...
	\includegraphics[scale=0.60]{sumandosPrimos100.jpg}
